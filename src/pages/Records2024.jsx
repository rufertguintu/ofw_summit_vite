...
const RECORD_ENDPOINTS = [
  "/wp-json/custom/v1/2024-records",
  "/wp-json/custom/v1/2024_userdata",
  "/wp-json/custom/v1/2024-user-data",
];
...
const getStatus = (record) => pickFirstValue(record, ["status", "admin_verified"]) || "N/A";
const getRegistrantType = (record) => pickFirstValue(record, ["registrant_type", "type_registrant"]);
const getOfwType = (record) => pickFirstValue(record, ["ofw_type"]);
const getRegisteredDate = (record) => pickFirstValue(record, ["user_registered", "registered_date", "created_at"]);
const getSourceYear = (record) => pickFirstValue(record, ["source_year", "year"]);

async function fetchRecordsFromEndpoints(searchQuery, page) {
  let lastError = null;

  for (const endpoint of RECORD_ENDPOINTS) {
    const response = await fetchApi(
      `${endpoint}?search=${encodeURIComponent(searchQuery)}&page=${page}&per_page=${PAGE_SIZE}`
    );
    ...
    lastError = new Error(payload?.message || `Unable to load 2024 records from ${endpoint}.`);
  }

  throw lastError || new Error("Unable to load 2024 records.");
}

async function fetchRecordDetails(recordId, preferredEndpoint) {
  const endpoints = preferredEndpoint
    ? [preferredEndpoint, ...RECORD_ENDPOINTS.filter((endpoint) => endpoint !== preferredEndpoint)]
    : RECORD_ENDPOINTS;

  let lastError = null;

  for (const endpoint of endpoints) {
    const response = await fetchApi(`${endpoint}/${recordId}`);
    const payload = await response.json();
    ...
    lastError = new Error(payload?.message || `Unable to load 2024 record details from ${endpoint}.`);
  }

  throw lastError || new Error("Unable to load 2024 record details.");
}

function Records2024() {
  ...
  const [activeEndpoint, setActiveEndpoint] = useState(RECORD_ENDPOINTS[0]);
  ...
  useEffect(() => {
    ...
    fetchRecordsFromEndpoints(searchQuery, page)
      ...
      .catch((fetchError) => {
        ...
        setTotalPages(1);
        setTotalRecords(0);
        setError(fetchError.message || "Unable to load 2024 records.");
      })
      .finally(() => {
        if (isMounted) {
          setLoading(false);
  ...
  const handleViewRecord = async (recordId) => {
    if (!recordId) {
      ...
      setSelectedRecord({
        summary: {},
        record: { error: "Unable to determine the user ID for this record." },
      });
      return;
    }
    ...
    try {
      const payload = await fetchRecordDetails(recordId, activeEndpoint);
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              <form className="flex w-full max-w-2xl flex-col gap-3 sm:flex-row" onSubmit={handleSearchSubmit}>
                <div className="relative flex-1">
                  <label htmlFor="records-2024-search" className="sr-only">
                    Search 2024 records
                  </label>
                  <input
                    id="records-2024-search"
                    type="text"
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    type="button"
                    className="rounded-lg border border-table-line px-4 py-2 font-medium text-foreground"
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-table-line">
              <thead className="bg-muted">
                <tr>
                  <th className="px-6 py-3 text-start text-xs font-medium uppercase text-muted-foreground-1">Source Year</th>
              ...
              <tbody className="divide-y divide-table-line">
                {loading ? (
                  <tr>
                    <td colSpan="11" className="px-6 py-8 text-center text-sm text-foreground">
                      Loading 2024 records...
                    </td>
                  </tr>
                ) : error ? (
                  <tr>
                    <td colSpan="11" className="px-6 py-8 text-center text-sm text-red-600">
                      {error}
                    </td>
                  </tr>
                ) : records.length === 0 ? (
                  ...
                  records.map((record) => {
                    ...
                    return (
                      <tr key={`${recordId}-${getEmailAddress(record)}`}>
                        <td className="whitespace-nowrap px-6 py-4 text-sm text-foreground">{getSourceYear(record) || "N/A"}</td>
                        <td className="whitespace-nowrap px-6 py-4 text-sm font-medium text-foreground">{recordId || "N/A"}</td>
